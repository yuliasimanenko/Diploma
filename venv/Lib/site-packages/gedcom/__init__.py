...
line_format = re.compile(r'^.?(?P<level>[0-9]+)\s+((?P<id>@[-a-zA-Z0-9]+@)\s+)?(?P<tag>[_A-Z0-9]+)(\s+(?P<value>.*))?$')
...
class Element(object):
    ...
    def __getitem__(self, key):
        ...
        if len(children) == 0:
            return None #raise IndexError(key)
        elif len(children) == 1:
            return children[0]
        elif len(children) > 1:
            return children
    ...
    def __contains__(self, key):
        """
        Return True if there is at least one child element with this tag, False otherwise.
    ...
    def gedcom_lines(self):
        ...
        """
        line_format = re.compile("^.?(?P<level>[0-9]+)\s+((?P<id>@[a-zA-Z0-9]+@)\s+)?(?P<tag>[A-Z]+)(\s+(?P<value>.*))?\s+$")
...
class Event(Element):
    ...
    def birth_day(self):
        if self['DATE']:
            ...
            day = re.search(r'[1-9] |1[0-9]| 2[0-9]|3[0-1]', date_str)
            month = re.search(r'[A-Z]{3}', date_str)
            year = re.search(r'\d{4}', date_str)

            date_dict = {}
            if day:
                date_dict['day'] = int(day.group(0))
            if month:
                date_dict['month'] = month.group(0)
            if year:
                date_dict['year'] = int(year.group(0))
            return date_dict
        else:
            return None
